...
URL 構造: https://furusato.mynavi.jp/products/search?category_id={id}&page={p}
...
BASE_URL = "https://furusato.mynavi.jp"

# (category_id, category_label) — マイナビふるさと納税のカテゴリID体系
...
def _extract_item(card: ElementHandle, category: str) -> dict | None:
    ...
    title_el = (
        card.query_selector(".p-productCard__name")
    ...
    price_el = (
        card.query_selector(".p-productCard__price")
    ...
    muni_el = (
        card.query_selector(".p-productCard__city")
...
def _scrape_page(page: Page, cat_id: str, category: str, p: int) -> list[dict]:
    url = f"{BASE_URL}/products/search?category_id={cat_id}&page={p}"
    page.goto(url, wait_until="domcontentloaded", timeout=45_000)

    try:
        page.wait_for_selector("a[href*='/products/']", timeout=15_000)
    except Exception:
        log.debug("マイナビ cat=%s p=%d: 商品カード未検出", category, p)
        return []

    cards = (
        page.query_selector_all(".p-productCard")
            ...
            for cat_id, cat_name in CATEGORIES:
                for p in range(1, pages_per_category + 1):
                    try:
                        ...
                        total += n
                        log.info("マイナビ cat=%s p=%d: %d件 upsert", cat_name, p, n)
                    except Exception as e:
                        log.warning("マイナビ cat=%s p=%d エラー: %s", cat_name, p, e)
                        break
                    self.sleep()

            browser.close()

        log.info("マイナビ 完了: 合計 %d件", total)
        return total


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    MynaviScraper().run_sync()
