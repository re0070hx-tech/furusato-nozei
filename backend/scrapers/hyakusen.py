...
URL 構造: https://furusato100sen.com/products?category_id={id}&page={p}
...
BASE_URL = "https://furusato100sen.com"
...
def _scrape_page(page: Page, cat_id: str, category: str, p: int) -> list[dict]:
    ...
    try:
        ...
        log.debug("ふるさと百選 cat=%s p=%d: 商品カード未検出", category, p)
        return []
...
class HyakusenScraper(BaseScraper):
    site_name = "ふるさと百選"
    site_id   = "hyakusen"
            ...
            for cat_id, cat_name in CATEGORIES:
                for p in range(1, pages_per_category + 1):
                    try:
                        ...
                        total += n
                        log.info("ふるさと百選 cat=%s p=%d: %d件 upsert", cat_name, p, n)
                    except Exception as e:
                        log.warning("ふるさと百選 cat=%s p=%d エラー: %s", cat_name, p, e)
                        break
                    self.sleep()

            browser.close()

        log.info("ふるさと百選 完了: 合計 %d件", total)
        return total


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    HyakusenScraper().run_sync()
