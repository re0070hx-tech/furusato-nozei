...
URL 構造: https://www.furusato-yell.com/products?category_id={id}&page={p}
...
BASE_URL = "https://www.furusato-yell.com"
...
def _scrape_page(page: Page, cat_id: str, category: str, p: int) -> list[dict]:
    ...
    try:
        ...
        log.debug("エール cat=%s p=%d: 商品カード未検出", category, p)
        return []
            ...
            for cat_id, cat_name in CATEGORIES:
                for p in range(1, pages_per_category + 1):
                    try:
                        ...
                        total += n
                        log.info("エール cat=%s p=%d: %d件 upsert", cat_name, p, n)
                    except Exception as e:
                        log.warning("エール cat=%s p=%d エラー: %s", cat_name, p, e)
                        break
                    self.sleep()
